...
export default function ReportsPage() {
  ...
  const cols: Record<string, any[]> = {
    ...
    payments: [{ key: "paymentDate", label: t("date"), render: (p: any) => formatDate(p.paymentDate) }, { key: "customer", label: t("customer"), render: (p: any) => p.customer?.name }, { key: "detail", label: t("detail") }, { key: "amount", label: t("amount"), render: (p: any) => <span className="font-medium">{p.amount?.toLocaleString("en-US")}</span> }, { key: "paymentMethod", label: t("method") }, { key: "destination", label: t("to"), render: (p: any) => p.destination === "haji" ? "Haji" : t("in_hand") }],
    expenses: [{ key: "expenseDate", label: t("date"), render: (e: any) => formatDate(e.expenseDate) }, { key: "detail", label: t("detail") }, { key: "amount", label: t("amount"), render: (e: any) => <span className="text-red-600 font-medium">{e.amount?.toLocaleString("en-US")}</span> }],
    haji_settlement: [{ key: "transferDate", label: t("date"), render: (tr: any) => formatDate(tr.transferDate) }, { key: "detail", label: t("detail") }, { key: "amount", label: t("amount"), render: (tr: any) => <span className="text-orange-600 font-medium">{tr.amount?.toLocaleString("en-US")}</span> }, { key: "transferType", label: t("type") }],
