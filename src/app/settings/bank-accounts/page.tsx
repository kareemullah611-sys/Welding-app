...
export default function BankAccountsPage() {
  ...
  const handleCreate = async () => {
    ...
    setSubmitting(true);
    const r = await apiCall("/api/v1/bank-accounts", { method: "POST", body: form });
    setSubmitting(false);
  ...
  const openEdit = (acc: any) => {
    setSelected(acc);
    setForm({ bankName: acc.bankName, accountNumber: acc.accountNumber || "" });
  ...
  const handleEdit = async () => {
    if (!form.bankName.trim()) { setError("Bank name is required"); return; }
    setSubmitting(true);
    const r = await apiCall(`/api/v1/bank-accounts/${selected.id}`, { method: "PATCH", body: form });
    setSubmitting(false);
    if (r.success) { setShowEdit(false); load(); } else { setError(r.error || "Failed"); }
  };

  const toggleActive = async (acc: any) => {
    const action = acc.isActive ? "deactivate" : "reactivate";
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        action={
          <button onClick={openCreate} className="btn-primary text-sm">+ {t("new_bank_account")}</button>
        }
      />
      ...
      <DataTable
        columns={[
          ...
          },
          {
            ...
            render: (acc: any) => acc.accountNumber ? <span className="font-mono text-sm text-gray-600">{acc.accountNumber}</span> : <span className="text-gray-300">—</span>,
          },
          {
