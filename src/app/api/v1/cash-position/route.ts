...
export const GET = withAuth(async (request: NextRequest, context, user: JWTPayload) => {
  try {
    ...
    const [cashIn, chequeIn, bankIn, onlineIn] = await Promise.all([
    ...
    const totalInHand = Number(cashIn._sum.amount || 0) + Number(chequeIn._sum.amount || 0) + Number(bankIn._sum.amount || 0) + Number(onlineIn._sum.amount || 0);
