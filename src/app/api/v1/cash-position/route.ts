...
export const GET = withAuth(async (request: NextRequest, context, user: JWTPayload) => {
  try {
    ...
    const [cashIn, chequeIn, bankIn, onlineIn, openingCash, openingCheques] = await Promise.all([
      prisma.payment.aggregate({ where: { ...inHandFilter, paymentMethod: "cash" }, _sum: { amount: true } }),
      prisma.payment.aggregate({ where: { ...inHandFilter, paymentMethod: "cheque" }, _sum: { amount: true } }),
      ...
      prisma.openingCheque.aggregate({ where: cityFilter, _sum: { amount: true } }),
    ]);
    ...
    const [expenses, withdrawals, hajiTransfers] = await Promise.all([
      ...
      prisma.personalWithdrawal.aggregate({ where: cityFilter, _sum: { amount: true } }),
      prisma.hajiTransfer.aggregate({ where: { ...cityFilter, transferType: "from_in_hand" } as any, _sum: { amount: true } }),
    ]);

    const openingChequeTotal = Number(openingCheques._sum.amount || 0);
    const totalInHand = Number(openingCash._sum.amount || 0) + Number(cashIn._sum.amount || 0) + openingChequeTotal + Number(chequeIn._sum.amount || 0) + Number(bankIn._sum.amount || 0) + Number(onlineIn._sum.amount || 0);
    ...
    return successResponse({
      incomingToHand: {
        ...
        cheque: openingChequeTotal + Number(chequeIn._sum.amount || 0),
