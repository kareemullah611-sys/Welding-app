...
export const GET = withAuth(async (request: NextRequest, context: any, user: JWTPayload) => {
  try {
    ...
    // Treat soft-deleted records as not found
...
export const PUT = withAuth(async (request: NextRequest, context: any, user: JWTPayload) => {
  try {
    ...
    const updated = await prisma.expense.update({
      where: { id },
      data: { amount: body.amount || expense.amount, detail: body.detail || expense.detail, notes: body.notes !== undefined ? body.notes : expense.notes, updatedAt: new Date() },
    });
...
export const DELETE = withAuth(async (request: NextRequest, context: any, user: JWTPayload) => {
  try {
    ...
    // Soft delete — stamp deletedAt rather than removing the row so financial history is preserved
    ...
    await createAuditLog(
      user.userId, expense.cityId, "expenses", id, "delete",
      { amount: Number(expense.amount), detail: expense.detail }, undefined,
      getClientIP(request)
    );
