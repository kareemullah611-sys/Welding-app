...
export const PUT = withSuperAdmin(async (request: NextRequest, context: any, user: JWTPayload) => {
  try {
    ...
    const finalizedPeriod = await prisma.profitAttributionPeriod.findFirst({
      where: {
        status: "finalized",
    ...
    if (finalizedPeriod) {
      return errorResponse("FINALIZED_PERIOD", "This rate is covered by a finalized investor period. Reverse/correct the finalization before changing historical evidence.", 409);
    }
    ...
    return successResponse({ id, pkrExchangeRate: rate, pkrExchangeRateMetadata: nextMetadata }, "PKR exchange rate saved");
