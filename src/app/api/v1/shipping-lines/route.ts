...
export const GET = withSuperAdmin(async (request: NextRequest, _context, _user: JWTPayload) => {
  try {
    ...
    const result = await Promise.all(lines.map(async (sl) => {
      const totalCostsUsd = await prisma.lotCost.aggregate({
        where: { shippingLineId: sl.id, currencyCode: "USD" },
        _sum: { amount: true },
      });
      const totalPaidUsd = await prisma.shippingLinePayment.aggregate({
        where: { shippingLineId: sl.id },
        _sum: { amountUsd: true },
      });
      const billed = Number(totalCostsUsd._sum.amount || 0);
