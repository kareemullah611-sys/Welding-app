...
export const POST = withSuperAdmin(async (request: NextRequest, _context, user: JWTPayload) => {
  try {
    ...
    const { shippingLineId, lotId, paymentDate, amountUsd, exchangeRate, reference, notes } = body;

    if (!shippingLineId || !paymentDate || !amountUsd) {
      return validationError("shippingLineId, paymentDate, and amountUsd are required");
    }
