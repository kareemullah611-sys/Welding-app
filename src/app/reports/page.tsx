...
export default function ReportsPage() {
  ...
  const cols: Record<string, any[]> = {
    sales: [{ key: "saleDate", label: t("date") }, { key: "voucherNo", label: t("voucher") }, { key: "customer", label: t("customer"), render: (s: any) => s.customer?.name }, { key: "status", label: t("status"), render: (s: any) => s.status === "marked_short" ? <span className="text-xs px-1.5 py-0.5 rounded bg-yellow-50 text-yellow-700">⚠️ Short</span> : <span className="text-xs px-1.5 py-0.5 rounded bg-green-50 text-green-700">Active</span> }, { key: "totalAmount", label: t("amount"), render: (s: any) => <span className="font-medium">{s.totalAmount?.toLocaleString()}</span> }],
    payments: [{ key: "paymentDate", label: t("date") }, { key: "customer", label: t("customer"), render: (p: any) => p.customer?.name }, { key: "detail", label: t("detail") }, { key: "amount", label: t("amount"), render: (p: any) => <span className="font-medium">{p.amount?.toLocaleString()}</span> }, { key: "paymentMethod", label: t("method") }, { key: "destination", label: t("to"), render: (p: any) => p.destination === "haji" ? "Haji" : t("in_hand") }],
    expenses: [{ key: "expenseDate", label: t("date") }, { key: "detail", label: t("detail") }, { key: "amount", label: t("amount"), render: (e: any) => <span className="text-red-600 font-medium">{e.amount?.toLocaleString()}</span> }],
    haji_settlement: [{ key: "transferDate", label: t("date") }, { key: "detail", label: t("detail") }, { key: "amount", label: t("amount"), render: (tr: any) => <span className="text-orange-600 font-medium">{tr.amount?.toLocaleString()}</span> }, { key: "transferType", label: t("type") }],
    customer_ledger: [{ key: "date", label: t("date") }, { key: "type", label: t("type"), render: (e: any) => <span className={`text-xs px-1.5 py-0.5 rounded ${e.type === "sale" ? "bg-blue-50 text-blue-700" : "bg-green-50 text-green-700"}`}>{e.type}</span> }, { key: "currency", label: t("currency"), render: (e: any) => <span className="text-xs font-semibold text-gray-500">{e.currency}</span> }, { key: "detail", label: t("detail") }, { key: "debit", label: t("debit"), render: (e: any) => e.debit ? <span className="text-red-600">{e.debit.toLocaleString()}</span> : "" }, { key: "credit", label: t("credit"), render: (e: any) => e.credit ? <span className="text-green-600">{e.credit.toLocaleString()}</span> : "" }, { key: "balance", label: t("balance"), render: (e: any) => { const b = e.balance; return <span className="font-medium">{(typeof b === "number" && !isNaN(b)) ? b.toLocaleString() : "-"}</span>; } }],
    city_ledger: [{ key: "date", label: t("date") }, { key: "type", label: t("type"), render: (e: any) => <span className={`text-xs px-1.5 py-0.5 rounded ${e.type === "sale" ? "bg-blue-50 text-blue-700" : e.type === "payment" ? "bg-green-50 text-green-700" : e.type === "expense" ? "bg-red-50 text-red-700" : "bg-orange-50 text-orange-700"}`}>{e.category || e.type}</span> }, { key: "description", label: t("description"), className: "max-w-xs truncate" }, { key: "currency", label: t("currency"), render: (e: any) => <span className="text-xs font-semibold text-gray-500">{e.currency}</span> }, { key: "debit", label: t("debit"), render: (e: any) => (e.debit && !isNaN(e.debit)) ? <span className="text-red-600">{Number(e.debit).toLocaleString()}</span> : "" }, { key: "credit", label: t("credit"), render: (e: any) => (e.credit && !isNaN(e.credit)) ? <span className="text-green-600">{Number(e.credit).toLocaleString()}</span> : "" }, { key: "runningCashInHand", label: t("cash_in_hand"), render: (e: any) => <span className="font-medium">{(e.runningCashInHand != null && !isNaN(e.runningCashInHand)) ? Number(e.runningCashInHand).toLocaleString() : "—"}</span> }],
    discount_history: [
      { key: "discountDate", label: t("date") },
      { key: "customer",     label: t("customer"),    render: (d: any) => d.customer?.name },
      { key: "saleVoucherNo",label: t("sale_voucher") },
      { key: "saleDate",     label: t("sale_date") },
      { key: "lotNumber",    label: t("lot") },
      { key: "discountAmount", label: t("discount"), render: (d: any) => <span className="font-medium text-yellow-700">{d.currency?.symbol} {d.discountAmount?.toLocaleString()}</span> },
    ...
    <div>
      ...
      {summary && <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {reportType === "city_ledger" ? <>
          {Object.entries(summary.cashInHand || {}).map(([cc, v]: [string, any]) => <StatsCard key={`cash-${cc}`} title={`${t("cash_in_hand")} (${cc})`} value={`${cc} ${formatNumber(v)}`} icon="💰" color="green" />)}
          ...
          {Object.entries(summary.totalHajiOwed || {}).map(([cc, v]: [string, any]) => <StatsCard key={`haji-${cc}`} title={`${t("owed_to_haji")} (${cc})`} value={`${cc} ${formatNumber(v)}`} icon="↗️" color="yellow" />)}
        ...
        : reportType === "customer_ledger" ? <><StatsCard title={t("customer")} value={summary.name} icon="👤" color="blue" />{summary.balanceByCurrency && Object.keys(summary.balanceByCurrency).length > 0 ? Object.entries(summary.balanceByCurrency).map(([cc, amt]: [string, any]) => <StatsCard key={cc} title={`${t("balance")} (${cc})`} value={`${cc} ${formatNumber(Math.abs(amt || 0))}`} icon={amt > 0 ? "📋" : "✅"} color={amt > 0 ? "red" : "green"} />) : <StatsCard title={t("balance")} value={formatNumber(typeof summary.balance === "number" && !isNaN(summary.balance) ? Math.abs(summary.balance) : 0)} icon={summary.balance > 0 ? "📋" : "✅"} color={summary.balance > 0 ? "red" : "green"} />}</>
        : reportType === "discount_history"
          ...
          : <><StatsCard title={t("records")} value={formatNumber(summary.count)} icon="📄" color="blue" /><StatsCard title={t("total")} value={formatNumber(summary.total)} icon="💰" color="green" />{summary.haji !== undefined && <StatsCard title={t("to_haji")} value={formatNumber(summary.haji)} icon="↗️" color="yellow" />}{summary.inHand !== undefined && <StatsCard title={t("in_hand")} value={formatNumber(summary.inHand)} icon="💰" color="green" />}</>}
      </div>}
