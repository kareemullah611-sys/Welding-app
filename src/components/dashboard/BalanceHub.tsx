...
import { formatDate, PaginationBar, SkeletonLine } from "@/components/ui";
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-[#e4e4e7] bg-[#f4f4f5]/95 text-left text-[11px] font-semibold uppercase tracking-[0.12em] text-gray-500">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.key} className="border-b border-[#e4e4e7] transition-colors last:border-b-0 hover:bg-[#fafafa]">
              ...
              <td className="whitespace-nowrap px-3 py-2.5 text-right tabular-nums text-rose-700">
...
const LedgerLoading = () => (
  <div className="space-y-3 px-4 py-3">
    {Array.from({ length: 4 }).map((_, index) => (
      <SkeletonLine key={index} delay={index * 90} className="h-4 w-full" />
    ))}
  </div>
);
...
const BranchRow = ({
  ...
  <div className="border-t border-gray-100">
    <button
      type="button"
      onClick={onToggle}
      className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50 sm:px-5"
    >
      <div className={cn("flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg", chip || "bg-gray-100 text-gray-600")}>
        <Icon className="h-4 w-4" />
      </div>
      <span className="text-sm font-medium text-gray-800">{label}</span>
      <span className="ml-auto text-sm font-semibold tabular-nums text-gray-900">{value}</span>
      <ChevronDown className={cn("h-4 w-4 flex-shrink-0 text-gray-400 transition-transform duration-300", open ? "" : "-rotate-90")} />
    </button>
  ...
  return (
    ...
    >
      <button
        ...
        className="flex w-full items-center gap-3.5 px-5 py-4 text-left transition-colors hover:bg-gray-50/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40 focus-visible:ring-inset"
            ...
            >
              <div className="space-y-1 px-2 py-1">
                {(treasury?.bankAccounts || []).map((acct) => (
                  <div key={acct.id} className="overflow-hidden rounded-lg bg-white ring-1 ring-gray-100">
                    <button
                      type="button"
                      onClick={() => {
