...
type ReceivablesReportLike = { totalByCurrency?: Record<string, number>; customers?: ReceivableRow[] };
...
export function applyPendingReceivablesReport(
  ...
  const totalByCurrency = { ...(baseData.totalByCurrency || {}) };
  ...
  for (const queued of queuedItems) {
    ...
    if (queued.url === "/api/v1/sales") {
      ...
      addBucketTotal(totalByCurrency, currency, amount);
      addLedgerRow(customers, account, currency, amount);
      continue;
    }
  ...
  return { ...baseData, totalByCurrency, customers };
}
...
export function applyPendingPayablesReport(
  ...
  return { ...baseData, suppliers, agents, shippingLines };
}
